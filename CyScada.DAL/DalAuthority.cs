...
namespace CyScada.DAL
{
    public class DalAuthority
    {
        ...
        {
            var sqlBuilder = new StringBuilder(@"SELECT * FROM  ZQ_Authorities WITH(NOLOCK) where 1=1 ");
        ...
        {
            var sql = string.Format(@"INSERT INTO  ZQ_Authorities
...
'{2}','{3}'", model["Name"], model["Description"], model["AuthorityCode"], model["AuthorityType"]);
        ...
        {
            var sql = string.Format(@"UPDATE   ZQ_Authorities
SET     Name = '{1}' ,
        Description = '{2}' 
WHERE   ID = {0}", model["Id"], model["Name"], model["Description"]);
        ...
        {
            var sql = string.Format(@"DELETE FROM  ZQ_Authorities WHERE ID={0}", id);
