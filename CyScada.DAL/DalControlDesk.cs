...
namespace CyScada.DAL
{
    public class DalControlDesk
    {
        ...
        {
            var sql =
                string.Format(@"SELECT * FROM  ZQ_SideMenu WITH(NOLOCK) WHERE MenuType = 1  AND ParentId = " +
                              sideMenuId + " order by sortnumber;SELECT * FROM  ZQ_SideMenu WITH(NOLOCK) WHERE Id = " + sideMenuId);
