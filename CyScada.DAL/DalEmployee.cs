...
namespace CyScada.DAL
{
    public class DalEmployee
    {
        ...
        {
            var sqlBuilder = new StringBuilder(@"SELECT * FROM  ZQ_Employees WITH(NOLOCK) where 1=1 ");
        ...
        {
            var sql = string.Format(@"INSERT INTO  ZQ_Employees
        ( Name ,
          LoginName ,
          Password ,
          Description ,
...
VALUES  ( '{0}' , -- Name - varchar(100)
        ...
        {
            var sql = string.Format(@"UPDATE   ZQ_Employees
SET     Name = '{1}' ,
        LoginName = '{2}' ,
        Password = '{3}' ,
        Description = '{4}' ,
...
WHERE   ID = {0}", model["Id"], model["Name"], model["LoginName"], model["Password"], model["Description"],
        ...
        {
            var sql = string.Format(@"DELETE FROM  ZQ_Employees WHERE ID={0}", id);
