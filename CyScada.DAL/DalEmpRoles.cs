...
namespace CyScada.DAL
{
    public class DalEmpRoles
    {
        ...
        {
            var sqlBuilder = new StringBuilder(@"SELECT * FROM  ZQ_EmpRoles WITH(NOLOCK) where 1=1 ");
        ...
        {
            var sql = string.Format(@"INSERT INTO  ZQ_EmpRoles
        ( emp_id, role_id )
VALUES  ( {0}, -- emp_id - int
          {1}  -- role_id - int
        ...
        {
            var sql = string.Format(@"DELETE FROM  ZQ_EmpRoles WHERE emp_id={0} AND role_id={1}", model["UserId"], model["RoleId"]);
            try
            {
                var result = SqlHelper.ExecuteNonQuery(SqlHelper.GetConnection(), CommandType.Text, sql);
