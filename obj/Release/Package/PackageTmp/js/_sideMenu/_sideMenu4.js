...
function AppendBranch(branchHtml, branch) {
    if (branch.SubMenus==undefined || branch.SubMenus.length == 0) {
        branchHtml.push(leafTemplate.replace('@url', branch.Url).replace('@url', branch.Url).replace('@name', branch.Name).replace('@class', branch.Class).replace('@id', 'menuId=' + branch.Id).replace('@id', 'menuId=' + branch.Id));
    } else {
        branchHtml.push(branchTemplate.replace('@url', branch.Url).replace('@url', branch.Url).replace('@name', branch.Name).replace('@class', branch.Class).replace('@id', 'menuId=' + branch.Id).replace('@id', 'menuId=' + branch.Id));
