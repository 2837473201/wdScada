...
namespace CyScada.DAL
{
    public class DalMapping
    {
        ...
        {
            var sql = @"SELECT * FROM  ZQ_MappingTable WITH(NOLOCK) where MappingType='" +
                      mappingType + "' ";
        ...
        {
            var sql = string.Format(@"INSERT INTO  ZQ_MappingTable
...
VALUES  ( '{0}', -- Text - varchar(max)
        ...
        {
            var sql = string.Format(@"UPDATE  ZQ_MappingTable
