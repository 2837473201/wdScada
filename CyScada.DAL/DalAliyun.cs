...
FROM  ZQ_Machine WITH(NOLOCK)
INNER JOIN  ZQ_MachineTags WITH(NOLOCK) ON ZQ_MachineTags.MachineId = ZQ_Machine.Id
INNER JOIN  aliyun WITH(NOLOCK) ON aliyun.[_NAME]=ZQ_MachineTags.DeviceName
WHERE 1=1 ");
